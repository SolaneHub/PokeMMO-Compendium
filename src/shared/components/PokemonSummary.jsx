...
const PokemonSummary = ({ pokemon, allPokemon, onClose, onSelectPokemon }) => {
  ...
  return (
    ...
    >
      <div
        className="relative flex h-[85vh] w-full max-w-[480px] animate-[scale-in_0.4s_ease-out_forwards] flex-col overflow-hidden rounded-xl border border-white/10 bg-[#1a1b20] shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div
          className="z-10 flex shrink-0 items-center justify-between p-4 text-slate-200 shadow-md"
          style={{ background }}
        >
        ...
        <div className="relative flex shrink-0 flex-col items-center border-b border-white/5 bg-[#0f1014] p-5">
          <button className="absolute top-4 right-4 rounded-md border border-white/10 bg-[#1a1b20] px-3 py-1.5 text-xs font-semibold text-slate-300 transition-colors hover:border-blue-600 hover:text-white">
            🔊 Cry
          </button>
          <img
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`relative min-w-[80px] flex-1 py-3.5 text-sm font-semibold whitespace-nowrap transition-colors ${
                activeTab === tab
                  ? "bg-[#1a1b20] text-blue-400 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-full after:bg-blue-600 after:content-['']"
                  : "text-slate-500 hover:bg-[#1a1b20] hover:text-slate-300"
              }`}
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto bg-[#1a1b20] p-5">
          ...
          >
            <div className="flex flex-col gap-2.5">
              <p className="m-0 rounded-lg border-l-4 border-blue-600 bg-white/5 p-3 text-sm leading-relaxed text-slate-200 italic">
                {pokemon.description}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <div className="flex gap-2.5">
                {pokemon.abilities?.main?.map((a) => (
                  <div
                    key={a}
                    className="flex-1 rounded-md border border-white/5 bg-[#0f1014] p-2 text-center text-sm text-slate-200"
                  >
                    {a}
                  </div>
                ))}
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="flex flex-col justify-center rounded-lg border border-white/10 bg-white/5 p-2.5">
                  ...
                  <span className="flex gap-2 text-sm font-semibold text-slate-200">
            ...
            <div className="flex flex-col gap-2.5">
              <div className="grid grid-cols-2 gap-2">
                <div className="col-span-2 flex flex-row items-center justify-between rounded-lg border border-white/10 bg-white/5 p-2.5">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">
                    Held Item
                  </span>
                  <span className="rounded bg-blue-600/10 px-2 py-0.5 text-sm font-bold text-blue-400">
                  ...
                  return statOrder.map((key) => {
                    ...
                    return (
                      <div key={key} className="mb-1 flex items-center">
                        ...
                        <span className="mr-2.5 w-9 text-right text-sm font-bold text-slate-200">
                          {val}
                        </span>
          ...
          >
            <div className="flex flex-col gap-2.5">
              <div className="mb-2 flex items-center justify-between border-b border-white/5 pb-2">
                ...
                <input
                  type="text"
                  className="w-44 rounded-md border border-white/10 bg-[#0f1014] px-3 py-1.5 text-sm text-slate-200 transition-all outline-none placeholder:text-slate-600 placeholder:italic focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20"
            ...
            <div className="flex flex-col gap-5">
              {variants.length > 0 && (
                <div className="flex flex-col gap-2.5">
                  ...
                  <div className="scrollbar-thin scrollbar-thumb-slate-700 flex items-center gap-0.5 overflow-x-auto rounded-lg border border-white/10 bg-[#0f1014] p-4">
                    {variants.map((variantName) => {
                      ...
                      return (
                        <div
                          key={variantName}
                          className="flex min-w-[80px] flex-1 cursor-pointer flex-col items-center justify-center rounded-lg border border-transparent p-1.5 transition-all hover:-translate-y-0.5 hover:border-blue-600 hover:bg-white/5"
                          onClick={() => onSelectPokemon(variantObj)}
                        >
                          <div className="mb-1.5 flex h-16 w-16 items-center justify-center rounded-full border border-white/5 bg-black/20">
                            <img
                          ...
                          <span className="text-center text-sm font-bold whitespace-nowrap text-slate-200">
                            {variantName}
                          </span>
              ...
              <div className="flex flex-col gap-2.5">
                ...
                {pokemon.evolutions && pokemon.evolutions.length > 0 ? (
                  <div className="scrollbar-thin scrollbar-thumb-slate-700 flex items-center justify-center gap-0.5 overflow-x-auto rounded-lg border border-white/10 bg-[#0f1014] p-4">
                      ...
                      return (
                        <Fragment key={evo.name}>
                          ...
                          {index < pokemon.evolutions.length - 1 && (
                            <div className="mx-0.5 shrink-0 text-lg font-bold text-blue-400 opacity-80">
                              ➜
                            </div>
                          )}
                        </Fragment>
...
const InfoCard = ({
  ...
  <div
    className={`flex flex-col justify-center rounded-lg border border-white/10 bg-white/5 p-2.5 ${className}`}
  >
    <span

...
interface EnemyPoolProps {
  selectedMember: EliteFourMember | null;
  enemyPool: string[];
  teamStrategies: Record<string, Record<string, StrategyStep[]>> | undefined;
  selectedEnemyPokemon: string | null;
  onSelectEnemy: (pName: string) => void;
  onAddEnemy: () => void;
  onRemoveEnemy: (pName: string, e: React.MouseEvent) => void;
}
...
const EnemyPool = ({
  ...
  return (
    <div className="animate-fade-in mt-6 rounded-xl border border-white/5 bg-[#1a1b20] p-5 text-white">
      ...
      <div className="custom-scrollbar grid max-h-[300px] grid-cols-1 gap-2 overflow-y-auto pr-1">
        {enemyPool.length === 0 ? (
          ...
          enemyPool.map((pName) => {
            const hasStrategy =
              (teamStrategies?.[selectedMember.name]?.[pName]?.length || 0) > 0;
            return (

        ...
        <section className="rounded-lg bg-white/5 p-3">
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(80px,1fr))] gap-2">
            {currentRaid.teamStrategies.map((strategy, idx) => (
              <button
                key={strategy.id || idx}
                className={`cursor-pointer rounded-md border border-white/5 bg-[#0f1014] p-2 text-sm font-semibold text-slate-400 transition-all hover:bg-white/10 hover:text-slate-200 ${selectedStrategyIndex === idx ? "border-blue-500/50 bg-blue-600/20 text-blue-400 shadow-[0_0_0_1px_rgba(59,130,246,0.1)]" : ""} `}
                onClick={() => {
            ...
            {movesForSelectedRole.length > 0 && (
              <>
                <div className="flex items-center justify-between rounded-t-lg border border-b-0 border-white/5 bg-white/5 p-2">
                  <button
                    className="flex h-8 w-8 items-center justify-center rounded-md border border-white/10 bg-[#0f1014] text-slate-200 transition-all hover:border-blue-500 hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:border-white/10 disabled:hover:bg-[#0f1014]"

...
import { RaidStrategy } from "@/pages/raids/types/raids";

interface RaidStrategyTabProps {
  currentRaid: Raid;
  selectedStrategyIndex: number;
  setSelectedStrategyIndex: (index: number) => void;
  setSelectedRole: (role: string | null) => void;
  setSelectedTurnIndex: (index: number | ((prev: number) => number)) => void;
  setSelectedBuildGroup: (group: string | null) => void;
  rolesSource: Record<string, string[]> | null;
  roleOptions: string[];
  effectiveSelectedRole: string;
  handleRoleChange: (roleKey: string) => void;
  movesForSelectedRole: string[];
      ...
      {currentRaid.teamStrategies && currentRaid.teamStrategies.length > 1 && (
        <section className="rounded-lg bg-white/5 p-3">
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(80px,1fr))] gap-2">
            {currentRaid.teamStrategies.map(
              (strategy: RaidStrategy, idx: number) => (
                <button
                  key={strategy.id || idx}
                  className={`cursor-pointer rounded-md border border-white/5 bg-[#0f1014] p-2 text-sm font-semibold transition-all hover:bg-white/10 ${

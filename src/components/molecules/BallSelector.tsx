...
import { BALL_TYPES } from "@/constants/calculatorConstants";

interface BallSelectorProps {
  selectedBall: string;
  onSelect: (ballName: string) => void;
}

const BallSelector = ({ selectedBall, onSelect }: BallSelectorProps) => {
        ...
        <button className="group flex w-full items-center justify-between rounded-xl border border-slate-700 bg-[#15161a] p-3 transition-all hover:border-slate-500">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col items-start text-white">
              <span className="font-bold">{selectedBall}</span>
        ...
        <div className="border-b border-slate-700 p-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-700 bg-[#15161a] py-2 pr-3 pl-9 text-sm text-white focus:border-blue-500 focus:outline-none"
              autoFocus
            />
          </div>
        </div>
        <div className="scrollbar-thin scrollbar-thumb-slate-700 max-h-60 overflow-y-auto p-1 text-white">
          {filteredBalls.map((ball) => (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-lg p-2 transition-colors ${selectedBall === ball.name ? "bg-blue-600/20 text-blue-400" : "hover:bg-white/5"}`}
            >
              <ItemImage item={ball.name} className="h-6 w-6 object-contain" />

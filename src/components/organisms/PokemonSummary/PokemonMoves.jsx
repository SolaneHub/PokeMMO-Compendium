  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="mb-2 flex items-center justify-between border-b border-white/5 pb-2">
        ...
        <input
          type="text"
          className="w-44 rounded-md border border-white/10 bg-[#0f1014] px-3 py-1.5 text-sm transition-all outline-none placeholder:text-slate-600 placeholder:italic focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20"
          ...
          filteredMoves.map((move, i) => (
            ...
            >
              ...
              <div className="w-10 text-center">
                {" "}
                <span className="group-hover: font-mono text-sm font-bold text-slate-400">
                  {" "}
                  {move.level}{" "}
                </span>{" "}
              </div>{" "}
              ...
              <div className="flex w-12 justify-center">
                {" "}
                <span
                  className={`w-10 rounded-sm py-0.5 text-center text-[9px] font-bold text-neutral-900 uppercase ${move.cat === "Physical" ? "bg-orange-400" : move.cat === "Special" ? "bg-sky-400" : "bg-neutral-400"}`}
                >
                  {" "}
                  {move.cat?.substring(0, 4)}{" "}
                </span>{" "}
              </div>{" "}
              <div className="w-10 text-center text-sm"> {move.pwr} </div>{" "}
              <div className="w-11 text-right text-sm"> {move.acc}% </div>{" "}
            </div>
          ))
        ) : (

...
function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-white/5 bg-[#1a1b20] transition-all duration-300 ease-in-out lg:static ${
          isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        } ${isCollapsed ? "lg:w-20" : "lg:w-64"} w-64 text-white`}
      >
        {/* Header */}
          ...
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div className="absolute top-1/2 left-0 h-8 w-1 -translate-y-1/2 rounded-r-full bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.5)]" />
                )}
                {/* Tooltip for Collapsed State */}
                {isCollapsed && (
                  <div className="absolute left-full z-50 ml-2 hidden rounded border border-white/10 bg-slate-800 px-2 py-1 text-xs whitespace-nowrap shadow-lg lg:group-hover:block">
                    {item.name}
                  </div>
                )}
              </NavLink>
                ...
                {!isCollapsed && (
                  <div className="flex flex-1 flex-col overflow-hidden">
                    ...
                    <Button
                      ...
                      className="h-auto w-fit p-0 text-red-400 hover:bg-transparent hover:text-red-300 hover:underline"
                    >

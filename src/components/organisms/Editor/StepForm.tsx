...
const StepForm = ({ step, onChange }: StepFormProps) => {
  usePokedexData();
  ...
  const update = (
    field: keyof StrategyStep,
    value: string | StrategyVariation[] | undefined
  ) => {
  ...
  const handleInsertTag = (tag: string) => {
    ...
    update("player", currentText + (currentText ? " " : "") + tag);
  };
    ...
    <div className="mt-2 space-y-4 text-white">
      ...
      <div className="relative">
        <textarea
          ...
          className="w-full resize-y rounded-xl border border-white/10 bg-[#0f1014] px-4 py-3 text-sm font-medium text-white transition-all outline-none focus:border-blue-500 focus:bg-[#0f1014]/50"
          ...
          <div className="space-y-4">
            ...
            {step.variations?.map((variation, i) => (
              <div key={i} className="relative pl-3">
                ...
                <VariationForm
                  variation={variation}
                  onChange={(upd: StrategyVariation) => {
